...
class DomainNetAug(ContinualDataset):
    NAME = 'domain-net-aug'
    SETTING = 'domain-il'
    ...
    def __init__(self, args):
        ...
        self.TRANSFORM = [transform_sobel_edge(args, args.shape_upsample_size, p=self.p),
    ...
    @staticmethod
    def get_backbone():
        return resnet18(DomainNetAug.N_CLASSES_PER_TASK)
    ...
    def get_norm_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), transforms.Compose(DomainNetAug.TRANSFORM_NORM)])
        return transform
