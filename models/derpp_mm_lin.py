...
class DerppMMLin(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            ...
            buf_outputs_ema1 = self.ema_lin(buf_inputs1)
            loss_aux_ema1 = self.aux.loss(buf_outputs1, buf_outputs_ema1.detach())
            buf_outputs_ema2 = self.ema_lin(buf_inputs2)
            loss_aux_ema2 = self.aux.loss(buf_outputs2, buf_outputs_ema2.detach())

            loss_log_12 = F.mse_loss(buf_outputs1, buf_logits1)
            loss_log_21 = F.mse_loss(buf_outputs2, buf_logits2)

            loss_buf1 = (loss_aux_ema1 * self.args.loss_wt[0]) + (self.args.alpha_mm[0] * loss_log_12)
            loss_buf2 = (loss_aux_ema2 * self.args.loss_wt[1]) + (self.args.alpha_mm[1] * loss_log_21)
            ...
            loss1 += loss_buf1 + (self.args.beta_mm[0] * loss_buf_ce1)
            loss2 += loss_buf2 + (self.args.beta_mm[1] * loss_buf_ce2)

        self.aux.collate_loss(loss_dict, loss_ce=loss_ce1, loss_buf_ce=loss_buf_ce1, loss_aux=loss_aux12, loss_aux_mem=loss_aux_ema1,
                              loss_logit_mem=loss_log_12, m1=True)
        self.aux.collate_loss(loss_dict, loss_ce=loss_ce2, loss_buf_ce=loss_buf_ce2, loss_aux=loss_aux21, loss_aux_mem=loss_aux_ema2,
    ...
    def update_ema_model_variables(self):
        ...
        for ema_param, param1, param2 in zip(self.ema_lin.parameters(), self.net.parameters(), self.net2.parameters()):
            ema_param.data = (alpha * param1.data) + ((1 - alpha) * (param2.data))
