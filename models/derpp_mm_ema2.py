...
class DerppMMEma2(ContinualModel):
    NAME = 'derpp_mm_ema2'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform, aux_transform=None):
        super(DerppMMEma2, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            ...
            buf_outputs_ema2 = self.ema_net2(buf_inputs2)
            ...
            if self.args.buf_aux:
                loss_aux_ema12 = self.aux.loss(buf_outputs1, buf_outputs_ema2.detach())
                loss_aux_ema21 = self.aux.loss(buf_outputs2, buf_outputs_ema1.detach())
                loss_buf1 += (loss_aux_ema12 * self.args.loss_wt[0])
                loss_buf2 += (loss_aux_ema21 * self.args.loss_wt[1])
